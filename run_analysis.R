...
# Merging the files by columns creates a record that includes the test subject ID (1-30), activity label (1-6), and accelerometer result.
# Repeat this merge with the testing files. 

training<-cbind(subject_train,y_train,x_train)
testing<-cbind(subject_test, y_test, x_test)

#1c. Read in column headers from features.txt, then create a vector with all column names to assign to training and testing datasets. 
#Because we merged the results files so that the test subject ID is in Column 1, activity label in Column 2,
...
#Reading in the feature names as a table creates a table with 2 columns, we will only pull from the column with the variable descriptions.
features<-features.names[,2]

#R automatically converts the text in feature.names to factors; we will convert these names into characters so we can read them as column headers.
features<-as.character(features)
all.cols<-c("SubjectID", "Activity", features)

#Now we'll assign the column header vector to each of the training and testing datasets.
names(testing)<-all.cols
names(training)<-all.cols

#1d. We can use rbind to merge the training and testing datasets because all columns follow the same order as eachother.
merged.dat<-rbind(training,testing)

#This completes Part 1 and Part 4 of the assignment.

#3a. Before tackling Part 2, we'll create a new column that explains each of the activity labels (1-6), using the activity_labels.txt file that's given to us.
# First, we'll read in the activity_labels.txt file.
...
table(merged.dat2$Activity) #This is the new column with descriptions replacing the 1-6's. Identical table results confirm that the loop worked.

#Relabeling is done. This completes Part 3.

#Next, we'll create a copy of mergeddat2 for column header cleanup.
...
#Now let's begin building our second tidy dataset by extracting only measurements of the mean or standard deviation of a measure.
...
#Now we'll average each measure by activity, for each subject, using plyr operations.
...
#Our last step will rename the column headers for our newly averaged values, such that each column name indicates an averaged value
...
#Let's apply this new vector of edited labels to the dataframe.
